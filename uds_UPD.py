...
class UDSClient:
    def __init__(self, config):
        ...
        self.bus = can.interface.Bus(
            ...
            can_filters=[{"can_id": self.rx_id, "can_mask": 0x7FF, "extended": is_extended}]
        )
        ...
        # Map DIDs to SafeAsciiCodec with lengths from config
    ...
    def run_testcase(self, oled):
            ...
            for tc_id, steps in grouped_cases.items():
                for step in steps:
                    ...
                    try:
                        ...
                        if service_int == 0x10:  # Diagnostic Session Control
                            response = client.change_session(subfunc_int)
                        ...
                        elif service_int == 0x27:  # SecurityAccess
                            # 1. Request Seed
                            seed_subfunc = subfunc_int
                            response = client.security_access(seed_subfunc)
                            ...
                            # 2. Send seed to PC via UDP
                            udp_ip = "192.168.1.100"  # PC IP address (change as needed)
                            udp_port = 5005  # PC UDP server port (change as needed)
                            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                            sock.settimeout(5)
        ...
        generate_report(
            ...
            report_name=f"UDS Testcase Report ({report_timestamp})",
            can_log=can_log_file,
            duration=wall_duration
        )

        self.stop_logging()

        oled.display_centered_text("Testcases Done\nReport generated")
        time.sleep(3)
