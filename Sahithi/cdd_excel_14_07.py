...
# Load all lines for step 3.3 use (for text-based TUV extraction)
...
for diagclass in root.xpath(".//DIAGCLASS"):
    ...
    for diaginst in diagclass.xpath(".//DIAGINST"):
        ...
        tmplref_v2 = ""
        if tmplref_v1:
            tmpl_elem = root.find(f'.//DCLSRVTMPL[@id="{tmplref_v1}"]')
            if tmpl_elem is not None:
                tmplref_v2 = tmpl_elem.get("tmplref", "")

        # Step 3.3: Find Service ID from <PROTOCOLSERVICE id=V2> → TUV tag
        service_id = ""
        if tmplref_v2:
